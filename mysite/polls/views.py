...
def vote1(request, question_id):
    ...
    if up2!=0.01:  # 负跌幅为1时
        ...
        print('money:',num_total*up1+num2_total*up2,(num_total*up1+num2_total*up2)/df_no_total.shape[0])
...
def vote(request, question_id):
    ...
    num2 = 0  # 小于-0.15的数量
    # 获取股价sql
    sql_high = r"""select date,open,close,high,low,up_change from '{}' where date>='{}' LIMIT {}"""
    sql_high_china = r"""select 日期,开盘,收盘,最高,最低,涨跌幅 from '{}' where 日期>='{}' LIMIT {}"""
    # 获取前一季度也预告的股票sql
    sql_continuity ="""SELECT * FROM polls_east_yj_yg as p WHERE
    p."股票代码" = '{}' AND
    p."业绩变动" LIKE '%{}%' and (
    ...
    for iii in quarter:
        ...
        for i,t in df.iterrows():
            dat1 = pd.read_sql(sql_continuity.format(t['股票代码'],yj_change2), conn )
    ...
    print(num,'{}内天至少有一天大于{}的概率:'.format(len,up1),num/df_no.shape[0],'----------------------')
    # print(num_total,num_total/df_up1.shape[0],'{}内天至少有一天大于{}的概率:'.format(len,up1),num_total/df_no_total.shape[0],'----------------------')
    if up2!=0.01:  # 负跌幅为1时
        print(num2,'{}内天小于{}的概率:'.format(len,up2),num2/df_no.shape[0])
