...
class AkStockPrice(object):
    ...
    def ak_stock_finance(code, save=''):
        ...
        if save == "y":
            with sqlite3.connect(tools.db_path) as conn:
                st_financial.to_sql(code + 'finance', con=conn,
    ...
    def wy_stock_price(code, save='', adjust=''):
        ...
        if save == "y":
            with sqlite3.connect(tools.db_path) as conn:
                stock_df.to_sql(code + adjust, con=conn,
    ...
    def df_stock_price_q_hfq(code, save='', adjust=''):
        ...
        stock_df = stock_df.drop(
            labels=['成交量', '成交额', '涨跌额', '换手率'], axis=1)  # 同时删除a,b列
        if save == "y":
            with sqlite3.connect(tools.db_path) as conn:
                stock_df.to_sql('df' + code + adjust, con=conn,
                                if_exists='replace', index=False)
    ...
    def east_finance(code, save=''):
        ...
        east_y = east_y[['SECURITY_CODE', 'REPORT_DATE',
                         'NOTICE_DATE', 'UPDATE_DATE', 'PARENT_NETPROFIT']]
        if save == "y":  # net_profit
            with sqlite3.connect(tools.db_path) as conn:
                east_y.to_sql(code + 'east_y_fin', con=conn,
                              if_exists='replace', index=False)
    ...
    def stock_yj_yg_em(save=''):
        ...
        li = ['20170331', '20170630', '20170930', '20171231', '20180331',
        ...
        for i in li:
            ...
            if save == "y":
                with sqlite3.connect(tools.db_path) as conn:
                    east_y.to_sql('east_yj_yg', con=conn,
    ...
    def stock_jg_dy_tj_em(save=''):
        ...
        if save == "y":
            with sqlite3.connect(tools.db_path) as conn:
                east_y.to_sql('jg_dy_tj_em', con=conn,
                              if_exists='append', index=False)
                # time.sleep(1.9)
    ...
    def stock_jg_dy_detail_em(save=''):
        ...
        if save == "y":
            with sqlite3.connect(tools.db_path) as conn:
                east_y.to_sql('jg_dy_detail_em', con=conn,
                              if_exists='append', index=False)
                # time.sleep(1.9)
    ...
    def stock_zh_a_gd_hs(save=''):
        ...
        """2022中报不齐全，http://data.eastmoney.com/gdhs/
        ...
        for i in li:
            ...
            east_y = east_y.drop(
                labels=['最新价', '涨跌幅', '区间涨跌幅'], axis=1)  # 同时删除a,b列
            # print(east_y)
            if save == "y":
                with sqlite3.connect(tools.db_path) as conn:
                    east_y.to_sql('east_gd_hs', con=conn,
                                  if_exists='append', index=False)
                    time.sleep(1.5)

    @tools.time_show  # 循环银行码
    def for_code(self):
        import time
        li = """1601398
                0000001
