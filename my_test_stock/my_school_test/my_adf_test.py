...
@gl_v.time_show
def history_test(code2, save=''):  #获取数据并保存数据库
    import akshare as ak
    # import time

    #连接数据库，如果不存在，则自动创建
    conn=sqlite3.connect(gl_v.db_path_my_school_test)
    ...
    # time.sleep(1.23)
    conn.close()
# history_test(code2='603368',save='')  # 柳药603368

def arch_test(tab):  # arch检验
...
def get_adfuller(tab):  # 时间序列平稳性单位根检验
    from statsmodels.tsa.stattools import adfuller
    
    with sqlite3.connect(gl_v.db_path_my_school_test) as conn:
        data = pd.read_sql(r"select 涨跌幅 from '{}'".format("hfq"+tab), conn)
        # data = pd.read_sql(r"select log_ from '{}' ORDER BY date DESC".format(tab), conn)
        data = data.dropna()
        # data = data.iloc[10:data.shape[0]]   # 全数据检验
        # data = data.iloc[data.shape[0]-122*1:data.shape[0]]   # 半年数据检验
        # data = data.iloc[data.shape[0]-122*2:data.shape[0]]   # 1年数据检验
        # data = data.iloc[data.shape[0]-122*4:data.shape[0]]
        # data = data.iloc[data.shape[0]-122*6:data.shape[0]-122*4]
        # data = data.iloc[data.shape[0]-122*10:data.shape[0]-122*4]
        # data = data.iloc[data.shape[0]-122*10:data.shape[0]-122*6]
...
"""
def get_ljungbox(tab):  # 白噪声检验-ljungbox检验
    ...
    with sqlite3.connect(gl_v.db_path_my_school_test) as conn:
        ...
        ljungbox_result = acorr_ljungbox(data,  lags=60, return_df=True, boxpierce=True,)  # 返回统计量和p值，lags为检验的延迟数
